...
class ReponseuserController extends AbstractController
{
    ...
    public function show(EntityManagerInterface $entityManager, $idReponse): Response
    {
        $reponses = $entityManager
            ->getRepository(Reponse::class)
            ->find($idReponse);
        $reclamation = $entityManager->getRepository(Reclamation::class)->find($reponses->getIdReclamation());
        $ciblerec = $entityManager->getRepository(User::class)->find($reclamation->getIdCibleReclamation());
